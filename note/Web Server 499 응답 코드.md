...
# 현재 환경에서는 특히 `$request_time`을 봐야 함
...
# 2. 두 번째 시나리오: nginx → WAS 연결 자체가 성립하지 않음
...
# 3. 세 번째 시나리오: 요청은 WAS까지 도달했지만 처리가 완료되지 않음
...
# 7. Web Server에서는 499의 `request_time`이 가장 중요
...
# 9. nginx error.log에서 반드시 찾을 메시지
...
# 12. WAS에서는 access.log보다 이걸 먼저 확인
