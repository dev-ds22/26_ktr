...
### 2-2. REQUIRED와 SUPPORTS 차이

#### 2-2-1. Propagation 5개 비교
...
## 5. SUPPORTS를 범용으로 쓰면 위험한 이유

### 5.1 메서드 실행 결과가 호출 위치에 따라 달라짐
...
### 5.3 rollback-for가 사실상 의미 없어질 수 있음
...
### 5.4 read-only가 DB 쓰기 차단 장치로 동작하지 않을 수 있음
...
## 7. read-only가 실제로 안 먹는 것처럼 보이는 이유

### 7.1 read-only는 기본적으로 힌트라서
...
### 7.2 SUPPORTS는 새 트랜잭션을 만들지 않기 때문에
...
### 7.3 외부 트랜잭션에 참여하면 내부 read-only 선언이 무시될 수 있음
...
## 8. MariaDB 10.6.15에서 read-only 관점
...
## 12. 범용 메서드 `*`에 read-only를 쓰면 위험한 이유
...
### 문제 1. 명명 규칙 누락 시 쓰기 메서드가 비트랜잭션으로 실행
...
### 문제 2. read-only가 쓰기 차단을 보장하지 않음
...
### 3.1 `SUPPORTS + read-only=true`는 조회 트랜잭션을 보장하지 않음
...
### 3.2 `read-only=true`가 쓰기 SQL 차단 장치가 아님
...
### 3.3 조회 메서드 내부에서 실수로 DML이 호출되어도 구조적으로 막기 어려움
...
### 3.4 외부 read-only 트랜잭션 안에서 쓰기 메서드를 호출하는 문제
...
### 3.5 같은 클래스 내부 호출은 AOP 트랜잭션이 적용되지 않을 수 있음
...
## 4. MariaDB 10.6.15에서 read-only 관련 동작
...
## 6. 개선안 1: 조회 메서드를 명시하고 `REQUIRED read-only=true` 사용
...
## 7. 개선안 2: DB 레벨 read-only 강제가 필요하면 `enforceReadOnly=true`
...
## 9. 개선안 4: 단순 조회 성능 우선이면 SUPPORTS 유지 가능
