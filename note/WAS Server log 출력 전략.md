...
# 2. 먼저 바로잡아야 할 중요한 부분: `logging.properties`
...
# 5. `server.out` — 정상 운영 로그가 아니라 비상 진단 로그
...
# 6. Application `log4j.xml`에서 가장 먼저 할 변경
...
# 8. 기존 파일 로그에서 색상 Pattern도 제거 권장
...
# 9. JBoss는 `FILE=INFO`, `CONSOLE=ERROR` 전략
...
# 11. 왜 CONSOLE을 완전히 제거하지 않고 ERROR를 남기는가
...
# 12. Application ERROR를 server.log에도 일부러 넣을 필요는 없음
...
# 13. `System.out.println()`과 `printStackTrace()`는 운영 코드에서 제거
...
# 14. 한 단계 더 좋은 Application Logger 구조
...
# 17. ConsoleAppender를 제거했는데도 Application 로그가 server.log에 남는다면
...
# 18. `server.out` 자체의 Rotation도 별도로 확인
