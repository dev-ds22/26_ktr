...
toc_sticky: true
date: "2026-09-11"
last_modified_at: "2026-09-11 09:40:51 +0900"
mermaid: false
---
...
## 3. 가장 먼저 해야 할 작업: `HttpSession` 사용현황 전수조사
...
## 7. 현재 시스템에서는 `Refresh Token` 때문에 Redis가 유용해짐
...
## 9. 그래서 Token을 Cookie에 넣는 구조도 존재
예:
```http
Set-Cookie:
ACCESS_TOKEN=eyJ...
...
## 10. Token을 Cookie에 저장하면 CSRF를 다시 고려해야 함
...
## 11. LocalStorage JWT는 현재 시스템에는 추천하지 않음
흔히 사용하는:
```javascript
...
## 15. 그러나 `모든 Access Token을 Redis에 저장`하는 것은 추천하지 않음
예:
```text
Access Token
   ↓
...
## 17. Spring Security 적용 시 기존 코드의 가장 큰 변화
현재:
```java
...
---
### 20-3. Phase 2. Spring Security만 먼저 적용
...
## 23. `JWT vs Opaque Token`도 먼저 선택해야 함
| 구분            | JWT      | Opaque Token |
| ------------- | -------- | ------------ |
| Token 자체 정보   | 있음       | 없음           |
