...
### 5-1. 하지만 도입 시 가장 많이 장애가 발생하는 부분이기도 함
...
### 26-2. Phase 2 — Spring Security Framework만 도입
```text
SecurityFilterChain
AuthenticationManager
AuthenticationProvider
...
### 26-3. Phase 3 — 인증 기준을 SecurityContext로 변경
기존:
```java
session.getAttribute("LOGIN_INFO")
```
...
### 26-6. Phase 6 — Login/Logout Handler 이전
```text
AuthenticationSuccessHandler
AuthenticationFailureHandler
AuthenticationEntryPoint
...
### 26-7. Phase 7 — 기존 인증 Filter/Interceptor 제거
안정화된 후:
```text
