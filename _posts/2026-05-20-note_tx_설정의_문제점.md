...
toc_sticky: true
date: "2026-05-20"
last_modified_at: "2026-05-20 18:06:11 +0900"
---
정확도: 96%
## 1. 결론
...
## 3. OrderService → OrderHistoryService 호출 시 조합별 동작

### 3-1. OrderService `REQUIRED` → OrderHistoryService `REQUIRED`
...
## 4. OrderService `REQUIRED` → OrderHistoryService `SUPPORTS`
...
## 5. OrderService `SUPPORTS` → OrderHistoryService `REQUIRED`
...
## 6. OrderService `SUPPORTS` → OrderHistoryService `SUPPORTS`
...
## 8. 현재 설정에서 특히 조심할 케이스

### 8-1. 상위 OrderService가 `SUPPORTS`인데 내부에서 OrderHistoryService 쓰기 호출
...
## 9. OrderService가 `SUPPORTS read-only=true`인데 OrderHistoryService가 쓰기
...
## 10. 외부 read-only 트랜잭션 안에서 OrderHistoryService 쓰기 호출
...
### 12-2. OrderHistoryService를 직접 생성
